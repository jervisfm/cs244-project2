...
packet and additively increase the window if the RTT is less than the threshold,
or additively decrease the window size if the measured RTT is
greater than the threshold.
The amount of increase and decrease are defined by the constant \texttt{alpha} and \texttt{beta}, respectively.
...
\subsection*{EWMA-estimated Receiver Rate}
...
datagrams. This served somewhat like an estimate of the bottleneck bandwidth
...
packets in flight at any given time as $RTT_{estimate} * BP_{estimate}$ using
the estimates that were calculated.
...
algorithm to reach a state where the estimated bandwidth was a rate of 1 packet
per RTT, which caused our algorithm to send a single packet at a time in an
attempt to not bloat the buffers, when really the estimate was incorrect.

This approach used the following parameters:
\texttt{max\_window\_size}, \texttt{min\_window\_size}, and \texttt{weight}.
We initialized our estimated RTT to $80$ ms, and our estimated receiver rate to
1 packet per 2.4 ms (approximately 5Mbps). The weight is used in updating the
average time between packets received by the receiver as
$time = (weight * measured_time) + ((1 - weight) * time)$
where $measured_time$ is the time interval between datagrams received, and the
rate is calculated as $1$ packet per $time$. This means that a larger weight
...
\begin{table}[!h]
  ...
  \begin{tabular}{rrrrrr}
    \toprule
    \textbf{max\_window\_size} & \textbf{min\_window\_size} & \textbf{weight} & \textbf{\specialcell{Throughput\\(Mbits/s)}} & \textbf{\specialcell{95\% Signal\\Delay (ms)}} & \textbf{Score}\\
    \midrule
  ...
  \end{tabular}
\end{table}
% TODO: Plot these in a particular color on a figure. At the end have 1 figure
% showing the dots of all our algorithms

As shown in Table~\ref{tab:attempt_1}, we were unable to break above a power
score of 20. Note that when \texttt{min\_window\_size} was set to 0, we see
dismal scores to do our algorithm getting in the undesirable state of sending
only a single packet at a time as described above. Furthermore, increasing
this minimum to 2 packets resulted in similar behavior, where 1 packet would be
sent per RTT/2, and not provide an accurate feedback signal for our estimates.
Once the minimum window was set to 3 packets, we were able to get better
...
recover from these larges drops, and would often take 5 or more seconds before
...
\subsection*{Delay-Based Additive Increase, Additive Decrease}
...
One limitation is that our algorithm assumes that the round trip propagation
delay does not change over the course of the trace. Our algorithm would not
be able to adjust to changes in propagation delay.
...
We call our approach \fbox{\texttt{Rocinante}}, named after Don Quixote's
noble steed.
