...
packet and increase the window size if the RTT is less than the threshold, or
decrease the window size if the measured RTT is greater than the threshold.
The amount of increase and decrease are defined by the constants
\texttt{win\_inc} and \texttt{win\_dec}, respectively. For example, if the
measured RTT is greater than \texttt{rtt\_thresh}, $cwnd = cwnd - win\_inc$.
This results in exponential increase and decrease of the window based on delay.

We experiment with several values of our three constants: \texttt{win\_inc},
\texttt{win\_dec}, and \texttt{rtt\_thresh}, and summarize the results in
Table~\ref{tab:delay}
...
\begin{table}[h]
  ...
  \begin{tabular}{rrrrrr}
    \toprule
    \textbf{win\_inc} & \textbf{win\_dec} & \textbf{rtt\_thresh} & \textbf{\specialcell{Throughput\\(Mbits/s)}} & \textbf{\specialcell{95\% Signal\\Delay (ms)}} & \textbf{Score}\\
    \midrule
    1 & 1              & 2             & 4.72          & 847          & 5.57           \\
    \bottomrule
  \end{tabular}
