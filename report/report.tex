...
network (by observing RTTs of individual packets). In the fixed window scheme,
no ajudstments were made for congestion, and in the AIMD scheme of the previous
exercise, the algorithm only reacted to timeouts (since our mahimahi
environment does not drop packets). Consequently, AIMD would still cause
...
We call our approach \fbox{\texttt{Rocinante}}, named after the light,
fast-attack, ship in The Expanse.
